...
#      Date : 12/21/2020
#             2025-02-05 transferred to package rda_python_dstacc from
#             https://github.com/NCAR/rda-utility-programs.git
#            2025-12-03 convert to class DsTACC
#   Purpose : check Backup flags set in dataset.backflag and/or dsgroup.backflag of RDADB
#             to gather Web and Saved tfiles, tar them into larger (> 2GB) tfiles, and
#             back them up onto TACC Backup Servers, at the Globus end points of
#             NCAR GDEX TACC and/or NCAR GDEX TACC Drdata, for Backup and/or
...
class DsTACC(PgCMD, PgSplit):
   ...
   IDSACT = 64  # hidden action to add dsids string to tfile records for addtional dsids
   MCSACT = 128  # hidden action to add MD5 checksum string to note fields of tfile records 
   DSCNT = 65
   DSTEP = 1000

   def __init__(self):
      super().__init__()  # initialize parent class
      self.BACKMSG = "TACC Backup"
      ...
      self.tfileS = {}    # cache current order number for each dsid
      self.BCKSIZE = 1024*self.PGLOG['ONEGBS']  # 90GB, dsglobus transfer size
      self.TARSIZE = 1024*self.PGLOG['ONEGBS']   # 1TB, tarfile size limit
      self.MINSIZE = 512*self.PGLOG['ONEGBS']    # .5TB, minimal tarfile size
   ...
   def read_parameters(self):   
      self.PGLOG['LOGFILE'] = "dstacc.log"   # set different log file
      self.dssdb_dbname()
      option = None
      argv = sys.argv[1:]
      for arg in argv:
         ms = re.match(r'^-(a|b|c|d|e|E|l|m|n|t|u|A)$', arg)
         if ms:
            arg = ms.group(1)
            if  arg == 'b':
      ...
      if not (self.sopts['a'] or self.dsids):
         print("dstacc [-b] [(-a|-t DatasetIDs)] [-c ChangeDays] [-(e|E)] [-(B|D)] [-A ActionBits] \\")
         print("         [-l (Y|N)] [-m ProcessCount] [-n] [-u] [-d [HostName] [TryCount]")
         print("  Option -a - gather files of all datasets for TACC Backup, or")
         ...
         print("              4 - Transfer TACC backup files; 8 - Check Backup Files;")
         ...
         sys.exit(0)
      self.PGBACK['cmd'] = "dstacc {}".format(' '.join(argv))

   # do tacc backup process
...
#         self.unlock_datasets()
   ...
   def add_to_dsids(self, dsid):
      if dsid.find('%') > -1:
         pgrecs = self.pgmget('dataset', 'dsid', "dsid LIKE '{}' AND taccflag = 'B'".format(dsid))
         if pgrecs:
            for dsid in pgrecs['dsid']:
               if dsid not in self.dsids: self.dsids.append(dsid)
      elif self.pgget('dataset', '', f"dsid = '{dsid}' AND taccflag <> 'B'"):
         self.pglog(dsid + ": Dataset ID is not for TACC Backup", self.LOGWRN)
      elif dsid not in self.dsids:
         self.dsids.append(dsid)

   # Check backup and file information and dump the backup statistics#
   def dump_statistics_action(self):   
      self.start_dstacc_none_daemon(self.STTACT)
      # files ready to be backed up
      dsfiles = {}
      self.gather_dataset_files(dsfiles, False)
      self.dump_dataset_files(dsfiles)
      ...
      self.dump_dataset_bckfiles(dsfiles)
   ...
   def check_backup_action(self):   
      ...
      if fcnt and self.dstart:
         self.start_dstacc_none_daemon(self.CHKACT, fcnt)
         self.process_dataset_chkfiles(dsfiles)
         self.process_dataset_chkfiles(dsfiles, 'D')

   # add Gnnn paths to old tfiles without ones
   def add_path_action(self):
      dsfiles = {}
      fcnt = self.gather_dataset_pathfiles(dsfiles)
      if fcnt and self.dstart:
         self.start_dstacc_none_daemon(self.PTHACT, fcnt)
         self.process_dataset_pathfiles(dsfiles)
         self.process_dataset_pathfiles(dsfiles, 'D')

   # add dsids to old tfiles without ones
   def add_dsids_action(self):
      dsfiles = {}
      fcnt = self.gather_dataset_dsidfiles(dsfiles)
      if fcnt and self.dstart:
         self.start_dstacc_none_daemon(self.IDSACT, fcnt)
         self.process_dataset_dsidfiles(dsfiles)
         self.process_dataset_dsidfiles(dsfiles, 'D')

   # add md5 checksums to old tfiles which miss the checksums in note field
   def add_checksum_action(self):
      dsfiles = {}
      fcnt = self.gather_dataset_mcsfiles(dsfiles)
      if fcnt and self.dstart:
         self.start_dstacc_none_daemon(self.MCSACT, fcnt)
         self.process_dataset_mcsfiles(dsfiles)
         self.process_dataset_mcsfiles(dsfiles, 'D')

   # check available GDEX files to create input files
   def create_infile_action(self):
      if self.dstart > 0: self.dstart = self.start_dstacc_none_daemon(self.CINACT)
      dsfiles = {}
      fcnt = self.gather_dataset_files(dsfiles)
      if fcnt and self.dstart:
         self.backup_dataset_files(dsfiles)
   ...
   def build_tarfile_action(self):
      ...
      if fcnt and self.dstart:
         if self.dstart > 0: self.dstart = self.start_dstacc_none_daemon(self.TARACT, fcnt)
         self.backup_dataset_infiles(dsfiles)
         self.backup_dataset_infiles(dsfiles, 'D')

   # gather and transfer tar files to Globus TACC Servers
   def globus_transfer_action(self):   
      dsfiles = {}
      fcnt = self.gather_dataset_tarfiles(dsfiles)
      if fcnt and self.dstart:
         if self.dstart > 0: self.dstart = self.start_dstacc_none_daemon(self.BCKACT, fcnt)
         self.backup_dataset_tarfiles(dsfiles)
         self.backup_dataset_tarfiles(dsfiles)

   # start none daemon and intialize dscheck counts
   def start_dstacc_none_daemon(self, act, fcnt = 0):
      acts = self.PGBACK['action']
      if self.bopts != None:
         ...
         self.init_dscheck(0, '', "dstacc", dsid, cact, self.PGBACK['workdir'],
         ...
         self.PGBACK['mproc'] = 1
      self.start_none_daemon('dstacc', '', self.PGLOG['CURUID'], self.PGBACK['mproc'], 60, 1)
      if self.PGLOG['DSCHECK']:
         if act == self.STTACT:
   ...
   def set_dstacc_progress(self, fcnt, fsize):
   ...
   # backup tfiles onto TACC Backup and/or Drdata endpoints for given datasets
   def backup_dataset_files(self, dsfiles):
      tfiles = dsfiles
      dcnt = len(tfiles)
      if not dcnt: return
      amsg = self.ACTMSG[self.PGBACK['action']&self.CTACTS]
      bmsg = self.BACKMSG
      dmsg = list(tfiles)[0] if dcnt == 1 else "{} datasets".format(dcnt)
      self.pglog("{}: {} for {} ...".format(amsg, bmsg, dmsg), self.WARNLG)
      qinfo = {'tid' : self.current_tid(), 'dsids' : [], 'fcnt' : 0,
...
#         fcnt = tfiles[dsid]['scount']
#         if fcnt > 0:
#            self.process_backup_files(qinfo, dsid, fcnt, tfiles[dsid]['srecs'], 'S')
         fcnt = tfiles[dsid]['wcount']
         if fcnt > 0:
            self.process_backup_files(qinfo, dsid, fcnt, tfiles[dsid]['wrecs'], 'W')
...
#            self.lock_dataset(dsid, 0, self.LGEREX)  # unlock the locked datasets
   ...
   # get the current tid for adding tfile
   def current_tid(self):
      pgrec = self.pgget("tfile", "max(tid) mid", '', self.LGEREX)
      return (pgrec['mid'] + 1) if pgrec else 1

   # get the current disp_order for adding tfile
   def display_order(self, dsid):
      if dsid not in self.ORDERS:
         pgrec = self.pgget('tfile', "max(disp_order) max_order", "dsid = '{}'".format(dsid), self.LGEREX)
         self.ORDERS[dsid] = pgrec['max_order'] if pgrec and pgrec['max_order'] else 0
      self.ORDERS[dsid] += 1
      return self.ORDERS[dsid]

   # backup tfiles with input files set onto TACC Backup and/or Drdata endpoints
   def backup_dataset_infiles(self, dsfiles):
      tfiles = dsfiles
      bcnt = len(tfiles)
      if not bcnt: return
...
#         if self.PGBACK['dolock']: self.lock_dataset(dsid, 0, self.LOGERR)
   ...
   # backup tfiles with tar files built onto TACC Backup and/or Drdata endpoints
   def backup_dataset_tarfiles(self, dsfiles):
      tfiles = dsfiles
      bcnt = len(tfiles)
      if not bcnt: return
      ...
      s = 's' if bcnt > 1 else ''
      self.pglog("{}: {} {} file{}...".format(amsg, bcnt, bmsg, s), self.WARNLG)
      qinfo = {'tids' : [], 'dsids' : [], 'fcnt' : 0, 'bcnt' : 0,
   ...
   def quit_dstacc(self, qinfo, msg = None):
...
#            for dsid in qinfo['dslocks']: self.lock_dataset(dsid, 0, self.LGEREX)
      ...
      if self.PGBACK['doemail']:
         ...
         title = "dstacc: Quit {} Error({})".format(amsg, self.PGBACK['errcnt'])
         if self.PGLOG['DSCHECK']:
            tbl = "dscheck"
            cnd = "cindex = {}".format(self.PGLOG['DSCHECK']['cindex'])
            self.build_customized_email(tbl, "einfo", cnd, title, self.LOGWRN)
   ...
   # backup one TACC Backup or Backup&Drdata from one or multiple inputs and,
   # reset the tacc backup dict
   def process_one_backup_file(self, qinfo, addback, keepid = False):
      ...
      if self.PGSIG['QUIT']: self.quit_dstacc(qinfo)
      dsids = qinfo['dsids']
      dcnt = len(dsids)
      if dcnt == 0: return
      ...
      fsize = qinfo['size']
      if addback:
         (tid, tfile) = self.add_backup_record(dsid, qinfo['tid'], dcnt, fcnt, fsize, dsids, qinfo['instr'])
      else:
         tid = qinfo['tid']
         tfile = qinfo['tfile']
      stat = 1
      if self.PGBACK['action'] > self.CINACT:
         ...
         qmsg = "{}: Try {} {} file for {}({}) file{} of {}".format(amsg, tfile, bmsg, fcnt, ssize, s, dmsg)
         cmd = "dsarch {} AT -TF {} ".format(dsid, tfile)
         cmd += '-TO -OE -MD -IF ' + ' '.join(qinfo['infiles'])
         if ccnt > 0:
            stat = self.start_child("dstacc_{}".format(tid), self.LOGWRN, 1)  # try to start a child process
            if stat <= 0:
               if self.PGSIG['QUIT']: self.quit_dstacc(qinfo)
               ...
               if stat:
                  ...
                  if self.pgdel('tfile', f"tid = {tid}"):
                     self.pglog(f"{dsid}-{tfile}: backup tarfile deleted for duplicattion", self.DTLACT)
                  for infile in qinfo['infiles']: self.delete_local_file(infile)
            ...
            if stat:
               ...
               if self.PGBACK['errcnt'] > self.PGBACK['maxcnt']: self.quit_dstacc(qinfo)
               time.sleep(self.PGSIG['ETIME'])
      if stat:
         # reset qinfo after tacc backup
         qinfo['qcnt'] += 1
         qinfo['qfcnt'] += fcnt
         qinfo['qsize'] += fsize
         for dsid in dsids:
...
#               qinfo['dslocks'].remove(dsid)
         ...
         qinfo['infiles'] = []
         qinfo['instr'] = ''
      if self.PGBACK['pstep']: self.set_dstacc_progress((fcnt if self.PGBACK['action']&self.CINACT else 1), fsize)

   # Transfer multiple tarfiles to TACC Backup or Backup&Drdata, and
   # reset the tacc backup dict
   def transfer_tacc_tarfiles(self, qinfo):
      ccnt = self.check_child(None, 0, self.LOGWRN, -1) if self.PGBACK['mproc'] > 1 else 0
      if self.PGSIG['QUIT']: self.quit_dstacc(qinfo)
      # prepare for backup one tar file
      dsids = qinfo['dsids']
      ...
      if ccnt > 0:
         stat = self.start_child("dstacc_{}".format(tids[0]), self.LOGWRN, 1)  # try to start a child process
         if stat <= 0:
            if self.PGSIG['QUIT']: self.quit_dstacc(qinfo)
            sys.exit(1)   # something wrong
         elif self.PGSIG['PPID'] <= 1:
      ...
      if ccnt == 0 or self.PGSIG['PPID'] > 1:
         ...
         if dstat and bstat:
            ...
            for tid in qinfo['tids']:
               self.pgexec("UPDATE tfile SET status = 'A' WHERE tid = {}".format(tid), self.LGEREX)
      ...
      qinfo['tofiles'] = []
      qinfo['fromfiles'] = []
      if self.PGBACK['pstep']: self.set_dstacc_progress((fcnt if self.PGBACK['action']&self.CINACT else bcnt), fsize)

   # add a backup file record as a place holder
   def add_backup_record(self, dsid, tid, dcnt, fcnt, fsize, dsids, instr):
      ...
      record = {'dsid' : dsid, 'tfile' : tfile, 'status' : 'N',
   ...
   def open_input_file(self, qinfo, dsid, ftype):
      infile = "{}_{}_{}.txt".format(dsid, ftype, qinfo['tid'])
   ...
   # build input files and backup a tacc file if accumulated enough files (>= self.TARSIZE)
...
#            if qinfo['size'] < self.MINSIZE: continue
   ...
   def get_dataset_files(self, dsid, dsfiles, logact = 0):
      ...
      if self.PGBACK['chgdays'] > 0:
         ...
         self.get_dataset_new_files(dsid, dcnd, tfiles, fopt, infiles, cnts)
      bcnt = cnts[0]
      if dsfiles is not None:
         cmsg = 'Changed ' if self.PGBACK['chgdays'] else ''
         if bcnt > 0:
            ssize = self.format_float_value(cnts[1])
            dsfiles[dsid] = tfiles
            bmsg = self.BACKMSG
            s = 's' if bcnt > 1 else ''
            self.pglog("{}: {}({}) {}file{} found for {}".format(dsid, bcnt, ssize, cmsg, s, bmsg), self.DTLACT)
      
         if bcnt == 0 and logact:
            self.pglog("{}: No {}file found to backup".format(dsid, cmsg), self.DTLACT)
      return bcnt

   # get new web/saved files to backup for given dataset/group info
   def get_dataset_new_files(self, dsid, dcnd, tfiles, fopt, infiles, cnts):
      if fopt&self.SOPT:
         cnd = dcnd
         cnd += ' AND tid = 0'
         self.get_dataset_saved_files('sfile', '*', cnd, tfiles, infiles, cnts)
      if fopt&self.WOPT:
         cnd = "tid = 0"
         self.get_dataset_web_files(dsid, None, '*', cnd, tfiles, infiles, cnts)

   # get changed web/saved files to re-backup for given dataset/group info
   def get_dataset_changed_files(self, dsid, dcnd, tfiles, fopt, infiles, cnts):
      self.file_backup_status(None)
      if fopt&self.SOPT:
         cnd = 'sfile.' + dcnd
         tables = 'sfile join tfile on sfile.tid = tfile.tid AND sfile.date_modified > tfile.date_modified'
         self.get_dataset_saved_files(tables, "tfile.tfile, sfile.*", cnd, tfiles, infiles, cnts)
      if fopt&self.WOPT:
         tjoin = 'join tfile on wfile.tid = tfile.tid AND wfile.date_modified > tfile.date_modified'
         self.get_dataset_web_files(dsid, tjoin, "tfile.tfile, wfile.*", '', tfiles, infiles, cnts)

   # get web data files for backup
   def get_dataset_web_files(self, dsid, tjoin, fields, cnd, tfiles, infiles, cnts):
      rcnt = 0 
      ...
      for i in range(cnt):
         ...
         fsize = pgrec['data_size']
         if tfiles:
            tfiles['wrecs'].append(pgrec)
            tfiles['wcount'] += 1
         cnts[0] += 1
         cnts[1] += fsize

   # get saved data files for backup
   def get_dataset_saved_files(self, tables, fields, cnd, tfiles, infiles, cnts):
      rcnt = 0 
      chgdays = self.PGBACK['chgdays']
   ...
   #   if self.pgget('sfile', 'sid', "{} AND tid {}".format(dcnd, bcnd), self.LGWNEX): fopt |= self.SOPT
      if self.pgget_wfile(dsid, 'wid', "tid " + bcnd, self.LGWNEX): fopt |= self.WOPT
      return fopt
   ...
   def gather_dataset_files(self, dsfiles, unlock = True):
      ...
      if self.dsids:
         for dsid in self.dsids:
            dcnd = "dsid = '{}'".format(dsid)
            pgrec = self.pgget("dataset", "dsid, pid", dcnd, self.LGWNEX)
            if pgrec:
#               if unlock and pgrec['pid'] and self.lock_dataset(dsid, 0, self.LOGACT) < 1: continue
               fcnt += self.get_dataset_files(dsid, dsfiles, self.LOGWRN)
      else:
         dcnd = "taccflag = 'B' ORDER BY dsid"
         pgrecs = self.pgmget("dataset", "dsid, pid", dcnd, self.LGWNEX)
         dcnt = len(pgrecs['dsid']) if pgrecs else 0
         for i in range(dcnt):
            dsid = pgrecs['dsid'][i]
#            if unlock and pgrecs['pid'][i] and self.lock_dataset(dsid, 0, self.LOGACT) < 1: continue
            fcnt += self.get_dataset_files(dsid, dsfiles)
      if dsfiles:
         s = 's' if fcnt > 1 else ''
         bmsg = self.BACKMSG
         cmsg = 'Changed ' if self.PGBACK['chgdays'] else ''
   ...
   def gather_dataset_infiles(self, dsfiles):
      ...
      bcnd = "status = 'N'"
      ocnd = " ORDER BY dsid"
      flds = "tid, dsid, tfile, type, data_size size, (scount + wcount) fcnt, dsids, note"
      if self.PGBACK['backflag']: bcnd += " AND type = '{}'".format(self.PGBACK['backflag'])
      if self.dsids:
         for dsid in self.dsids:
            dcnd = "dsid = '{}' AND {}".format(dsid, bcnd)
            pgrecs = self.pgmget("tfile", flds, dcnd, self.LGWNEX)
            bcnt = len(pgrecs['tid']) if pgrecs else 0
            for i in range(bcnt):
         ...
         pgrecs = self.pgmget("tfile", flds, bcnd+ocnd, self.LGWNEX)
         bcnt = len(pgrecs['tid']) if pgrecs else 0
         for i in range(bcnt):
            cnt = self.get_backup_infile(dsfiles, self.onerecord(pgrecs, i))
            if cnt:
   ...
   def gather_dataset_tarfiles(self, dsfiles):
      ...
      bcnd = "status = 'T'"
      ocnd = " ORDER BY dsid"
      flds = "tid, dsid, tfile, type, data_size size, (scount + wcount) fcnt, note"
      if self.PGBACK['backflag']: bcnd += " AND type = '{}'".format(self.PGBACK['backflag'])
      if self.dsids:
         for dsid in self.dsids:
            dcnd = "dsid = '{}' AND {}".format(dsid, bcnd)
   ...
   # get a backup file with input files set for tacc backup
   def get_backup_infile(self, dsfiles, pgrec):
      ret = 1
      tfile = pgrec['tfile']
      ms = re.search(r'([a-z]\d{6})_sn(\d+)_dn(\d+)_fn(\d+)', tfile)
      if ms:
         dsid = ms.group(1)
         tid = int(ms.group(2))
         dcnt = int(ms.group(3))
         fcnt = int(ms.group(4))
         if not (dsid == pgrec['dsid'] and tid == pgrec['tid']): return 0
      else:
         dsid = pgrec['dsid']
         tid = pgrec['tid']
         fcnt = pgrec['fcnt']
   ...
   #   if not (pgrec['note'] or self.rebuild_file_note(pgrec, dsids)): return 0
      if not pgrec['note']:
         return self.pglog(tfile + ": Miss note for N backup file", self.ERRACT)
      infiles = []
      flines = []
      fd = None
      ...
      size = pgrec['size']
      tfiles = dsfiles[bflg]
      if dsid not in tfiles: tfiles[dsid] = {}
      tfiles[dsid][tid] = {'tfile' : tfile, 'fcnt' : fcnt, 'size' : size, 'dsids' : dsids, 'infiles' : infiles}
      return ret

   # get a tarred backup file for tacc backup
   ...
   #   if not (pgrec['note'] or self.rebuild_file_note(pgrec, dsids)): return 0
      if not pgrec['note']:
         return self.pglog(tfile + ": Miss note for T backup file", self.ERRACT)
      ftype = None
      lines = pgrec['note'].split("\n")
      for line in lines:
      ...
      dsid = dsids[0]
      bflg = pgrec['type']
      fromfile = self.get_local_globus_file(ftype, tid, dsid, tfile, bflg)
      if not fromfile: return 0
      tofile = "/{}/{}".format(dsid, tfile)
      dsfiles[bflg][tid] = {'tfile' : tfile, 'fcnt' : fcnt, 'size' : pgrec['size'],
                            'dsids' : dsids, 'tofile' : tofile, 'fromfile' : fromfile}
      return ret
   
   # build fromfile name at Globus endpoint gdex-glade
   def get_local_globus_file(self, ftype, tid, dsid, tfile, backflag):
      endpath = 'decsdata' if ftype == 'S' else 'data'
      fromfile = "/{}/gdex-tacc/{}/{}".format(endpath, dsid, op.basename(tfile))
      tarfile = self.PGLOG['DSSDATA'] + fromfile
      ...
      self.pglog("{}: Miss local {} file for {}\n{}: Change file status to 'N' to recreate".format(fromfile, bmsg, amsg, tfile), self.DTLACT)
      self.pgexec("UPDATE tfile SET status = 'N' WHERE tid = {}".format(tid), self.LGEREX)
      return None

   # cache the Web/Saved files in type N tfiles (included in input files) already
   def cache_dataset_infiles(self, dsid):
      incnd = "status = 'N' AND (dsid = '{}' or dsids LIKE '%{}%')".format(dsid, dsid)
      pgrecs = self.pgmget('tfile', 'note', incnd, self.LGEREX)
   ...
   # process tfiles to check sizes between db and backup servers, set status to N if different
   def process_dataset_chkfiles(self, dsfiles, backflag):
      tfiles = dsfiles[backflag]
      bcnt = len(tfiles)
      if not bcnt: return
      ...
      s = 's' if bcnt > 1 else ''
      self.pglog("{}: {} {} file{}...".format(amsg, bcnt, bmsg, s), self.WARNLG)
      qinfo = {'backflag' : backflag, 'tid' : 0, 'dsid' : None, 'size' : 0, 'tfile' : None,
               'btfiles' : {}, 'dtfiles' : {}, 'qdsids' : [], 'qcnt' : 0, 'ncnt' : 0}
      for tid in tfiles:
         qinfo['tid'] = tid
         binfo = tfiles[tid]
         if qinfo['dsid'] and binfo['dsid'] != qinfo['dsid']:
            ...
            qinfo['dtfiles'] = {}
         for bkey in binfo: qinfo[bkey] = binfo[bkey]
         self.process_one_tacc_chkfile(qinfo)
      qcnt = qinfo['qcnt']
      if qcnt > 0:
         s = 's' if qcnt > 1 else ''
         ncnt = qinfo['ncnt']
   ...
   def process_one_tacc_chkfile(self, qinfo):
      if self.PGSIG['QUIT']: self.quit_dstacc(qinfo)
      # prepare for backup one tar file
      dsid = qinfo['dsid']
      tid = qinfo['tid']
      ...
      if bflg == 'D':
         ...
         tfiles = qinfo['dtfiles']
         if qpath not in tfiles: tfiles[qpath] = self.backup_glob(qpath, endpoint, 0, logact)
         info = tfiles[qpath][pfile] if tfiles[qpath] and pfile in tfiles[qpath] else None
         if not info:
            qmsg = "{}: Miss {} file {}-{}".format(amsg, bmsg, endpoint, tfile)
         elif dsize != info['data_size']:
            sqsize = self.format_float_value(info['data_size'])
            qmsg = "{}: Miss-match {} files {}({})/{}-{}({})".format(amsg, bmsg, dfile, sdsize, endpoint, tfile, sqsize)
      if not qmsg:
         endpoint = self.QPOINTS['B']
         tfiles = qinfo['btfiles']
         if qpath not in tfiles: tfiles[qpath] = self.backup_glob(qpath, endpoint, 0, logact)
         info = tfiles[qpath][pfile] if tfiles[qpath] and pfile in tfiles[qpath] else None
         if not info:
            qmsg = "{}: Miss {} file {}-{}".format(amsg, bmsg, endpoint, tfile)
      ...
      if qmsg:
         ...
         self.pgexec("UPDATE tfile SET status = 'N' WHERE tid = {}".format(tid), logact)
         qinfo['ncnt'] += 1
      # reset qinfo after tacc backup
      qinfo['qcnt'] += 1
      if dsid not in qinfo['qdsids']: qinfo['qdsids'].append(dsid)   
      if self.PGBACK['pstep']: self.set_dstacc_progress(1, dsize)

   # get all available backup files records for given datasets
   def gather_dataset_bckfiles(self, dsfiles, getnote = True):
      bcnd = "status = 'A'"
      ocnd = " ORDER BY dsid"
      FLDS = ['dsid', 'size', 'tfile']
      flds = "tid, dsid, tfile, type, data_size size"
      if getnote:
         FLDS.append('fcnt')
         FLDS.append('note')
         ...
         for dsid in self.dsids:
            ...
            if dsfiles:
               ...
               for i in range(bcnt):
                  ...
                  type = pgrec['type']
                  tid = pgrec['tid']
                  if tid not in dsfiles[type]:
                     dsfiles[type][tid] = {fld : pgrec[fld] for fld in FLDS}
                     fcnt += 1
            else:
               fcnt += self.pgget("tfile", '', dcnd, self.LGWNEX)
      else:
         if dsfiles:
            pgrecs = self.pgmget("tfile", flds, bcnd+ocnd, self.LGWNEX)
            fcnt = len(pgrecs['tid']) if pgrecs else 0
            for i in range(fcnt):
               pgrec = self.onerecord(pgrecs, i)
               dsfiles[pgrec['type']][pgrec['tid']] = {fld : pgrec[fld] for fld in FLDS}
         else:
            fcnt = self.pgget("tfile", '', bcnd, self.LGWNEX)
      if dsfiles:
         s = 's' if fcnt > 1 else ''
         amsg = self.ACTMSG[self.PGBACK['action']]
         bmsg = self.BACKMSG[self.PGBACK['backflag']] if self.PGBACK['backflag'] else 'backup'
   ...
   # process tfiles to add leading Gnnn path onto TACC Backup and/or Drdata endpoints
   def process_dataset_pathfiles(self, dsfiles, backflag):
      ...
      s = 's' if bcnt > 1 else ''
      self.pglog("{}: {} {} file{}...".format(amsg, bcnt, bmsg, s), self.WARNLG)
      qinfo = {'backflag' : backflag, 'tid' : 0, 'dsids' : [], 'fcnt' : 0, 'size' : 0,
               'tfile' : None, 'pfile' : None, 'qdsids' : [], 'qfcnt' : 0, 'qsize' : 0, 'qcnt' : 0}
      for tid in tfiles:
         qinfo['tid'] = tid
         binfo = tfiles[tid]
         for bkey in binfo: qinfo[bkey] = binfo[bkey]
         self.process_one_tacc_pathfile(qinfo)
      qcnt = qinfo['qcnt']
      if qcnt > 0:
         s = 's' if qcnt > 1 else ''
         fcnt = qinfo['qfcnt']
   ...
   # backup one TACC Backup or Backup&Drdata from one or multiple inputs and,
   # reset the tacc backup dict
   def process_one_tacc_pathfile(self, qinfo):
      ccnt = self.check_child(None, 0, self.LOGWRN, -1) if self.PGBACK['mproc'] > 1 else 0
      if self.PGSIG['QUIT']: self.quit_dstacc(qinfo)
      # prepare for backup one tar file
      dsids = qinfo['dsids']
      ...
      if ccnt == 0 or self.PGSIG['PPID'] > 1:
         ...
         if dstat and bstat:
            self.pgexec("UPDATE tfile SET tfile = '{}' WHERE tid = {}".format(pfile, tid), self.LGEREX)
         else:
            self.PGBACK['errcnt'] += 1
            if self.PGBACK['errcnt'] > self.PGBACK['maxcnt']: self.quit_dstacc(qinfo)
            time.sleep(self.PGSIG['ETIME'])
      ...
      if dstat and bstat:
         ...
         for dsid in dsids:
            if dsid not in qinfo['qdsids']: qinfo['qdsids'].append(dsid)
      if self.PGBACK['pstep']: self.set_dstacc_progress(1, fsize)

   # get all available backup file records without leading paths
   def gather_dataset_pathfiles(self, dsfiles):
      fcnt = 0
      bcnd = "status = 'A' AND tfile not LIKE 'G%/%.tar'"
      ocnd = " ORDER BY dsid"
      flds = "tid, dsid, tfile, type, data_size size, disp_order, (scount + wcount) fcnt, note"
      if self.PGBACK['backflag']: bcnd += " AND type = '{}'".format(self.PGBACK['backflag'])
      if self.dsids:
         for dsid in self.dsids:
            dcnd = "dsid = '{}' AND {}".format(dsid, bcnd)
   ...
   def get_backup_pathfile(self, dsfiles, pgrec):
      ...
      bflg = pgrec['type']
      dsfiles[bflg][tid] = {'dsids' : dsids, 'fcnt' : fcnt, 'size' : pgrec['size'], 'tfile' : tfile, 'pfile' : pfile}
      return 1

   # process tfile records to add dsids string
   def process_dataset_dsidfiles(self, dsfiles, backflag):
      ...
      for tid in tfiles:
         ...
         binfo = tfiles[tid]
         for bkey in binfo: qinfo[bkey] = binfo[bkey]
         self.process_one_tacc_dsidfile(qinfo)
      qcnt = qinfo['qcnt']
      if qcnt > 0:
         s = 's' if qcnt > 1 else ''
         fcnt = qinfo['qfcnt']
   ...
   # add dsids string to a tacc backup file record
   def process_one_tacc_dsidfile(self, qinfo):
      dsids = qinfo['dsids']
      tid = qinfo['tid']
      fcnt = qinfo['fcnt']
      fsize = qinfo['size']
      dsstr = ','.join(dsids[1:])
      stat = self.pgexec("UPDATE tfile SET dsids = '{}' WHERE tid = {}".format(dsstr, tid), self.LGEREX)
      if stat:
         # reset qinfo after tacc backup
         qinfo['qcnt'] += 1
         qinfo['qfcnt'] += fcnt
         ...
         for line in lines:
            ...
            if ms:
               ...
               if dsid not in dsids:
                  ...
                  if dcnt == len(dsids): break
      bflg = pgrec['type']
      dsfiles[bflg][tid] = {'dsids' : dsids, 'fcnt' : fcnt, 'size' : pgrec['size']}
      return 1

   # process tfile records to add md5 checksum strings
   def process_dataset_mcsfiles(self, dsfiles, backflag):
      ...
      qcnt = 0
      for tid in tfiles:
         qcnt += self.process_one_tacc_mcsfile(tid, tfiles[tid], qinfo)
      if qcnt > 0:
         s = 's' if qcnt > 1 else ''
         fcnt = qinfo['fcnt']
         mcnt = qinfo['mcnt']
   ...
   # add a md5 checksum string to a tacc backup file record
   def process_one_tacc_mcsfile(self, tid, binfo, qinfo):
      fields = 'tid, data_size size, checksum, date_modified date, time_modified time'
      bcnd = "tid = {}".format(tid)
      ...
      record = {'note' : '\n'.join(lines)}
      ret = 0
      if self.pgupdt('tfile', record, bcnd, self.LGEREX):
         qinfo['fcnt'] += binfo['fcnt']
         ...
         for dsid in dsids:
            if dsid not in qinfo['dsids']: qinfo['dsids'].append(dsid)
         ret = 1
      if self.PGBACK['pstep']: self.set_dstacc_progress(1, binfo['size'])
      return ret
   ...
   def gather_dataset_mcsfiles(self, dsfiles):
      ...
      ocnd = " ORDER BY dsid"
      flds = "tid, type, data_size size, (scount + wcount) fcnt, date_modified, time_modified, note"
      if self.dsids:
         for dsid in self.dsids:
            dcnd = "dsid = '{}' AND {}".format(dsid, bcnd)
            pgrecs = self.pgmget("tfile", flds, dcnd, self.LGWNEX)
   ...
   def get_backup_mcsfile(self, dsfiles, pgrec):
      dsfiles[pgrec['type']][pgrec['tid']] = {'note' : pgrec['note'], 'fcnt' : pgrec['fcnt'], 'size' : pgrec['size'],
   ...
   def rebuild_file_note(self, pgrec, dsids):
      ...
      tfiles = {dsids[0] : {'S' : [], 'W' : []}}
      pgrecs = self.pgmget('sfile', 's' + fields, "tid = {} ORDER BY dsid, sfile".format(tid), self.LGEREX)
      ...
      for i in range(cnt):
         ...
         if dsid not in dsids:
            dsids.append(dsid)
            tfiles[dsid] = {'S' : [], 'W' : []}
         tfiles[dsid]['S'].append([pgrecs['sfile'][i], pgrecs['type'][i], pgrecs['data_size'][i], pgrecs['checksum'][i]])
      pgrecs = self.pgmget('wfile', 'w' + fields, "tid = {} ORDER BY dsid, wfile".format(tid), self.LGEREX)
      ...
      for i in range(cnt):
         ...
         if dsid not in dsids:
            dsids.append(dsid)
            tfiles[dsid] = {'S' : [], 'W' : []}
         tfiles[dsid]['W'].append([pgrecs['wfile'][i], pgrecs['type'][i], pgrecs['data_size'][i], pgrecs['checksum'][i]])
      # create note
      note = ''
      for dsid in tfiles:
         for ftype in tfiles[dsid]:
            files = tfiles[dsid][ftype]
            if not files: continue
            note += "<{}_{}_{}.txt>\n".format(dsid, ftype, tid)
            note += "DS<=>{}\n{}F<:>{}T<:>SZ<:>MC<:>\n".format(dsid, ftype, ftype)
            for flist in files:
               note += "{}<:>{}<:>{}<:>{}<:>\n".format(flist[0], flist[1], flist[2], flist[3])
      if note:
         self.pgexec("UPDATE tfile SET note = '{}' WHERE tid = {}".format(note, tid), self.LGEREX)
         pgrec['note'] = note
         return 1
      return 0
   ...
   # TACC files backed up already and file counts deleted or moved
   def dump_dataset_bckfiles(self, dsfiles, backflag):
      ...
      for tid in tfiles:
         binfo = tfiles[tid]
         pcnt += self.count_changed_files(tid, binfo, qinfo)
         if self.PGBACK['pstep']:
            cnt += 1
            size += binfo['size']
            if cnt == self.DSTEP:
               self.set_dstacc_progress(cnt, size)
               cnt = size = 0
      if cnt: self.set_dstacc_progress(cnt, size)
   ...
   # count cache the Web/Saved files in type A tfiles (archived) already
   def count_changed_files(self, tid, binfo, qinfo):
      qinfo['fcnt'] += binfo['fcnt']
      qinfo['size'] += binfo['size']
      note = binfo['note']
      if not note:
      ...
      for fname in rfiles:
         ...
         if rinfo[0] == 'W':
            ...
            pgrec = self.pgget_wfile(rinfo[1], 'tid, data_size', cnd, self.LGEREX)
         else:
            cnd = "sfile = '{}' AND dsid = '{}'".format(fname, rinfo[1])
            pgrec = self.pgget('sfile', 'tid, data_size', cnd, self.LGEREX)
         if not pgrec:
            # deleted file
            qinfo['dcnt'] += 1
            qinfo['dsize'] += rinfo[2]
